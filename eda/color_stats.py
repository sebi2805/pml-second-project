...
def plot_hsv_histogram(hist_h, hist_s, hist_v, output_path):
    if hist_h.sum() > 0:
...
def main():
    ...
    for label in sorted(per_class_samples):
        ...
        if sum_image is None or image_count == 0:
            continue
        mean_image = (sum_image / image_count).round().astype(np.uint8)
